...
export const SignaturePad: React.FC<SignaturePadProps> = ({
  ...
  const handlePointerMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (!ctx) return;
    ctx.lineTo(x, y);
    ctx.strokeStyle = "#032E63"; // Corporate Navy Blue
    ctx.lineWidth = 2.5;
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.stroke();
    ...
    <div className="w-full relative flex flex-col">
      <div className="relative w-full h-[180px] bg-transparent">
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          className="absolute right-4 top-4 text-muted-foreground hover:text-primary hover:bg-gray-200/50 rounded-xl px-3 h-8 bg-gray-100/50 backdrop-blur-sm shadow-sm"
          onClick={handleClear}
        >
          Clear
        </Button>
