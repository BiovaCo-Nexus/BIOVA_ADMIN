...
export function AdminActivityLogs({ onNavigateToTab }: AdminActivityLogsProps = {}) {
 ...
 className={`p-4 transition-colors flex items-start gap-4 ${onNavigateToTab || log.action_type === 'DEMAND_TO_DIRECTORS' ? 'cursor-pointer hover:bg-muted/50/80' : 'hover:bg-muted/50/50'}`}
 >
 <div className="bg-primary text-primary-foreground text-foreground p-2 rounded-full mt-1">
 ...
 <span className="font-medium text-foreground">{log.action_type.replace(/_/g, ' ')}</span> on <span className="font-medium">{log.entity_name}</span>
 </p>
 {log.details && (
