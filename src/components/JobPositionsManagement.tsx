...
export function JobPositionsManagement() {
 ...
 <div className="space-y-4">
 <h3 className="text-lg font-bold text-foreground border-b pb-2">Existing Positions</h3>
 {positions.length === 0 ? (
 <div className="text-center py-8 text-gray-500">No job positions found. Create one above.</div>
 ) : (
 <div className="grid grid-cols-1 gap-4">
 {positions.map((position, index) => (
 <Card key={position.id} className=" shadow-sm hover: transition-shadow">
 <CardContent className="p-4 sm:p-5">
 <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
 <div className="space-y-3 flex-1 pr-0 sm:pr-4">
 <div className="flex flex-wrap items-center gap-2">
 <Briefcase className="h-5 w-5 text-foreground" />
 <h4 className="font-bold text-foreground text-lg leading-tight">{position.title}</h4>
 <div className="flex gap-2 flex-wrap mt-1 sm:mt-0">
 <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-md font-medium shrink-0">
 {position.job_type}
 </span>
 {position.is_active ? (
 <span className="bg-secondary/10 text-secondary-foreground text-xs px-2 py-1 rounded-md font-medium shrink-0">Active</span>
 ) : (
 ...
 <span className="bg-muted/50 text-foreground text-xs px-2 py-1 rounded-md font-medium shrink-0">Remote</span>
 )}
 </div>
 </div>
 ...
 <>
 <span className="text-gray-400">•</span>
 <span className="text-muted-foreground">{position.salary_range}</span>
 </>
 )}
 ...
 <Button size="sm" variant="outline" className="h-8 px-3 text-foreground border-border/20 hover:bg-primary hover:text-primary-foreground" onClick={() => handleEdit(position)}>
 Edit
 </Button>
