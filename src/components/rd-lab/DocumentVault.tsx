...
export function DocumentVault() {
  ...
  const getIconForCategory = (cat: string) => {
    switch (cat) {
      case "COA": return <ShieldCheck className="h-5 w-5 text-green-600" />
      case "FSSAI Docs": return <FileBadge className="h-5 w-5 text-blue-600" />
      case "MSDS": return <FileArchive className="h-5 w-5 text-amber-600" />
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[#032E63]">Document Vault</h2>
        {!isEditing && <Button onClick={() => { resetForm(); setIsEditing(true) }} className="bg-[#08A04B] hover:bg-[#069a43]"><Plus className="h-4 w-4 mr-2" />Add Document Link</Button>}
      </div>
      ...
      {isEditing ? (
        <Card className="border-l-4 border-l-[#08A04B] shadow-md">
          <CardHeader className="pb-3">
            <div className="flex justify-between items-center">
              <CardTitle className="text-[#032E63]">{form.id ? "Edit Document" : "New Document Record"}</CardTitle>
                ...
                <TableRow key={d.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-[#032E63]">{d.title}</div>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" asChild>
                      <a href={d.file_url} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4 text-green-600" /></a>
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => handleEdit(d)}><Edit className="h-4 w-4 text-blue-600" /></Button>
