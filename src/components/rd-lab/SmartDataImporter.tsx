...
export function SmartDataImporter() {
 ...
 <BrainCircuit className="w-32 h-32 text-foreground" />
 </div>
 ...
 className="border-2 border-dashed border-border bg-muted/50/50 rounded-xl p-8 text-center hover:bg-muted/50 cursor-pointer transition-all hover:border-border group"
 >
 <input 
 type="file" 
 accept=".csv" 
 ...
 <Loader2 className="h-12 w-12 text-foreground animate-spin mb-4" />
 <p className="font-bold text-foreground">AI is Analyzing Data...</p>
 <p className="text-xs text-gray-500 mt-2 animate-pulse">Mapping columns and auto-formatting...</p>
 </div>
 ) : result ? (
 <div className="flex flex-col items-center">
 <CheckCircle className="h-12 w-12 text-foreground mb-3" />
 <p className="font-bold text-xl text-foreground">Import Complete!</p>
 <div className="mt-4 p-4 bg-white rounded-lg border w-full max-w-sm text-left">
 ...
 <span key={c} className="text-[10px] bg-secondary text-foreground px-2 py-0.5 rounded-full">{c}</span>
 ))}
 </div>
 </div>
