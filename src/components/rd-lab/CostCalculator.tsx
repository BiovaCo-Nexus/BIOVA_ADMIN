...
export function CostCalculator() {
 ...
 <h2 className="text-xl font-bold text-foreground">Cost Calculator</h2>
 {!isEditing && <Button onClick={() => { resetForm(); setIsEditing(true) }} className="bg-primary text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4 mr-2" />New Calculation</Button>}
 </div>
 ...
 <div className="flex justify-between items-center">
 <CardTitle className="text-foreground">{form.id ? "Edit Calculation" : "New Cost Calculation"}</CardTitle>
 ...
 <h4 className="font-semibold text-foreground border-b pb-2">Direct Costs (₹)</h4>
 ...
 <div className="space-y-4">
 <h4 className="font-semibold text-foreground border-b pb-2">Final Pricing</h4>
 ...
 <Input type="number" step="0.1" value={form.margin_percent || 0} onChange={e => setForm({ ...form, margin_percent: parseFloat(e.target.value) || 0 })} className="bg-muted/50 border-border" /></div>
 <div className="pt-2 border-t">
 <label className="text-xs font-medium text-foreground uppercase tracking-wide">Suggested Selling Price</label>
 <div className="text-3xl font-black text-foreground">₹{(form.selling_price || 0).toLocaleString()}</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <TableCell><Badge variant="outline" className="bg-muted/50 text-foreground">{c.margin_percent}%</Badge></TableCell>
 <TableCell className="font-bold text-foreground">₹{(c.selling_price || 0).toLocaleString()}</TableCell>
 <TableCell className="text-right">
 <Button variant="ghost" size="icon" onClick={() => handleEdit(c)}><Edit className="h-4 w-4 text-foreground" /></Button>
