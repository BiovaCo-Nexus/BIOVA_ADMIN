  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[#032E63]">Ingredient Inventory</h2>
        {!isEditing && <Button onClick={() => { resetForm(); setIsEditing(true) }} className="bg-[#08A04B] hover:bg-[#069a43]"><Plus className="h-4 w-4 mr-2" />Add Stock</Button>}
      </div>
      ...
      {isEditing ? (
        <Card className="border-l-4 border-l-[#08A04B] shadow-md">
          <CardHeader className="pb-3">
            <div className="flex justify-between items-center">
              <CardTitle className="text-[#032E63]">{form.id ? "Edit Stock" : "New Stock Entry"}</CardTitle>
            ...
            <Table><TableHeader><TableRow>
              ...
              <TableBody>{inventory.map(item => {
                ...
                <TableRow key={item.id} className={isLow ? "bg-red-50/50" : ""}>
                  ...
                  <TableCell>
                    {isLow ? <Badge variant="outline" className="bg-red-100 text-red-700 border-red-200"><AlertTriangle className="h-3 w-3 mr-1" />Low Stock</Badge> : <Badge variant="outline" className="bg-green-100 text-green-700 border-green-200">In Stock</Badge>}
                  </TableCell>
