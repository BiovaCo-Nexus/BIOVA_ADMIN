  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-[#032E63]">Sample Management</h2>
        {!isEditing && <Button onClick={() => { resetForm(); setIsEditing(true) }} className="bg-[#08A04B] hover:bg-[#069a43]"><Plus className="h-4 w-4 mr-2" />Dispatch Sample</Button>}
      </div>
      ...
      {isEditing ? (
        <Card className="border-l-4 border-l-[#08A04B] shadow-md">
          <CardHeader className="pb-3">
            <div className="flex justify-between items-center">
              <CardTitle className="text-[#032E63]">{form.id ? "Edit Sample" : "New Sample Dispatch"}</CardTitle>
            ...
            <Table><TableHeader><TableRow>
              ...
              <TableBody>{samples.map(s => (
                <TableRow key={s.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline" className={s.status === 'delivered' ? "bg-blue-100 text-blue-700 border-blue-200" : s.status === 'feedback_received' ? "bg-green-100 text-green-700 border-green-200" : "bg-gray-100 text-gray-700"}>
