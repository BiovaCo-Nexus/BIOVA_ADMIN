...
export function ShelfLifeTesting() {
 ...
 <h2 className="text-xl font-bold text-foreground">Shelf Life Testing</h2>
 {!isEditing && <Button onClick={() => { resetForm(); setIsEditing(true) }} className="bg-primary text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4 mr-2" />Start Test</Button>}
 </div>
 ...
 <div className="flex justify-between items-center">
 <CardTitle className="text-foreground">{form.id ? "Edit Test" : "New Shelf Life Test"}</CardTitle>
 ...
 className={`h-8 text-xs shrink-0 ${selectedDay === day ? "bg-primary text-primary-foreground" : ""} ${hasData && selectedDay !== day ? "border-border bg-muted/50 text-foreground" : ""}`}
 ...
 <TableCell>
 <Badge variant="outline" className={t.status === 'running' ? "bg-primary text-primary-foreground text-foreground border-border" : t.status === 'completed' ? "bg-primary/10 text-foreground border-border" : "bg-red-100 text-red-700 border-red-200"}>
