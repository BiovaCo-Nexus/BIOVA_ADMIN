...
export function Contacts() {
  ...
  const [newItem, setNewItem] = useState({ first_name: "", last_name: "", email: "" })
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from('contacts').insert(newItem)
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" })
    } else {
      toast({ title: "Success", description: "Record added successfully." })
      setNewItem({ first_name: "", last_name: "", email: "" })
      fetchData()
    }
  }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div>
        <h2 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          <Users className="h-8 w-8 text-[#4B49AC]" /> Contact Directory
        </h2>
        <p className="text-gray-500 mt-2">Key individuals associated with your accounts.</p>
      </div>
