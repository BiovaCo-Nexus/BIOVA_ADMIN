...
export function Meetings() {
  ...
  const [newItem, setNewItem] = useState({ subject: "", meeting_date: "", status: "" })
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from('meetings').insert(newItem)
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" })
    } else {
      toast({ title: "Success", description: "Record added successfully." })
      setNewItem({ subject: "", meeting_date: "", status: "" })
      fetchData()
    }
  }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div>
        <h2 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          <Calendar className="h-8 w-8 text-[#4B49AC]" /> Meetings & Calls
        </h2>
        <p className="text-gray-500 mt-2">Schedule and log CRM activities.</p>
      </div>
      ...
      <Card className="border-gray-200">
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <Input placeholder="Date (YYYY-MM-DD HH:MM)" value={newItem.meeting_date} onChange={e => setNewItem({...newItem, meeting_date: e.target.value})} className="mb-2" />
            <Input placeholder="Status" value={newItem.status} onChange={e => setNewItem({...newItem, status: e.target.value})} className="mb-2" />
            </div>
