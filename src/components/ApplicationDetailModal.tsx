...
export function ApplicationDetailModal({ application, isOpen, onClose }: ApplicationDetailModalProps) {
 ...
 const statusColors = {
 submitted: "bg-primary text-primary-foreground text-foreground border-border",
 under_review: "bg-yellow-100 text-yellow-800 border-yellow-300",
 interview_scheduled: "bg-secondary text-foreground border-border",
 accepted: "bg-primary/10 text-foreground border-border",
 ...
 <CardTitle className="text-lg text-foreground">Deep AI Scan (OpenRouter)</CardTitle>
 </div>
 {!aiAnalysis && (
 <Button 
 ...
 <div className="relative w-20 h-20 rounded-full border-4 border-border flex items-center justify-center flex-shrink-0 bg-white shadow-inner">
 <span className={`text-2xl font-bold ${aiAnalysis.score >= 75 ? 'text-foreground' : aiAnalysis.score >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
 {aiAnalysis.score}%
 </span>
 </div>
 <div>
 ...
 <div className="bg-muted/50/50 p-4 rounded-lg border border-border">
 ...
 <span className="text-foreground mt-0.5">•</span> {pro}
 </li>
 ))}
 </ul>
 </div>
 ...
 <Sparkles className="h-8 w-8 text-foreground mx-auto mb-3" />
 ...
 <span className="font-semibold text-xs text-foreground">{note.admin_email}</span>
