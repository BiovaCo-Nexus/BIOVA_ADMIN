...
export const Model3DManagement = () => {
 ...
 <TableCell>
 <Badge className={model.is_active ? "bg-secondary text-secondary-foreground hover:bg-secondary text-secondary-foreground/90 text-white" : "bg-gray-400 text-white"}>
 ...
 {models.map((model) => (
 <Card key={model.id} className="p-4 flex flex-col gap-3">
 <div className="flex justify-between items-start">
 <div className="pr-4">
 <h3 className="font-bold text-foreground leading-tight mb-1">{model.title}</h3>
 <p className="text-xs text-gray-500 font-medium">
 {formatFileSize(model.file_size)} • {model.file_type.toUpperCase()}
 </p>
 </div>
 <Badge className={`shrink-0 ${model.is_active ? "bg-secondary text-secondary-foreground hover:bg-secondary text-secondary-foreground/90 text-white" : "bg-gray-400 text-white"}`}>
 {model.is_active ? "Active" : "Inactive"}
 </Badge>
 </div>
 ...
 {model.is_active ? <ToggleRight className="h-4 w-4 text-muted-foreground" /> : <ToggleLeft className="h-4 w-4 text-gray-400" />}
 </Button>
