...
export function ReceivablesPayables() {
  ...
  if (loading) return <div className="flex justify-center p-12"><Loader2 className="h-8 w-8 animate-spin text-purple-600" /></div>;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="border-t-4 border-t-rose-500 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-xl flex items-center justify-between">
              <span className="flex items-center text-rose-700"><ArrowRight className="w-5 h-5 mr-2 -rotate-90" /> Accounts Payable</span>
              ...
              <TableBody>
                {payables.map(po => (
                  <TableRow key={po.id}>
                    ...
                    <TableCell className="font-bold text-rose-600">₹{Number(po.total_amount - po.amount_paid).toLocaleString('en-IN')}</TableCell>
                    <TableCell>
